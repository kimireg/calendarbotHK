...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_auth(update): return
    msg = (
        "🤖 **Calendar Bot v2.6 (Task Edition)**\n\n"
        "我是你的 AI 日程秘书。你可以：\n"
        "1. **日程**: \"明天下午3点开会\"\n"
        "2. **任务**: \"记得买牛奶\" (自动设为全天)\n"
        "3. **发图**: 识别海报/机票\n"
        "4. **指令**: `/today`, `/event`, `/travel`, `/status`"
    )
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text_content and text_content.startswith("/event"):
        ...
        # 注意：如果发了图片但没写字，这里 text_content 为空是允许的，因为可能只需要 AI 分析图
    ...
    try:
        ...
        if update.message.photo:
            ...
            # 【修复点 3】：将用户输入的 caption (text_content) 发送给 AI
            # 如果用户没写字，默认提示词改为 "Extract event info from this image" 而不是简单的 "Analyze."
            ...
            resp = await client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": [
                        {"type": "text", "text": user_prompt}, # <--- 关键修改：把 caption 喂给 AI
        ...
        # --- 情况 B: 纯文本 ---
            ...
            if not text_content: return # 既没图也没字，直接忽略
...
if __name__ == '__main__':
    ...
    if tk := os.getenv("TELEGRAM_TOKEN"):
        ...
        print("✅ Calendar Bot v2.6 (Task & OpenRouter) Started...")
        app.run_polling()