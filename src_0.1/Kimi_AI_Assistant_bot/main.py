...
# 引入 Zeabur 远程控制 (确保 zeabur_remote.py 在同级目录)
...
# 优先读取环境变量，如果没有则使用默认值
MODEL_NAME = os.getenv("LLM_MODEL_NAME", "google/gemini-3-flash-preview")
...
def load_family_config():
    """
    从环境变量 FAMILY_CONFIG 读取 JSON 配置。
    如果未设置，使用默认的 (Chairman) 配置以保持向后兼容。
    """
    default_config = [
        {"name": "Kimi", "role": "Default / Father", "env_var": "GOOGLE_CALENDAR_ID", "icon": "👱‍♂️"},
        {"name": "Kiki", "role": "Daughter", "env_var": "GOOGLE_CALENDAR_ID_KIKI", "icon": "👧"},
        {"name": "Jason", "role": "Son", "env_var": "GOOGLE_CALENDAR_ID_JASON", "icon": "👦"},
        {"name": "Janet", "role": "Wife", "env_var": "GOOGLE_CALENDAR_ID_JANET", "icon": "👩‍🎨"}
    ]
    
    config_str = os.getenv("FAMILY_CONFIG")
    if config_str:
        try:
            return json.loads(config_str)
        except json.JSONDecodeError:
            logger.error("❌ FAMILY_CONFIG JSON 格式错误，使用默认配置")
            return default_config
    return default_config

# 加载配置
FAMILY_MEMBERS = load_family_config()

# 构建辅助数据结构
# 1. 有效分类集合 (自动添加 Family)
VALID_CATEGORIES = {m["name"] for m in FAMILY_MEMBERS}
VALID_CATEGORIES.add("Family")

# 2. 名字到日历ID环境变量名的映射
CATEGORY_TO_ENV = {m["name"]: m["env_var"] for m in FAMILY_MEMBERS}
CATEGORY_TO_ENV["Family"] = "GOOGLE_CALENDAR_ID_FAMILY" # Family 总是尝试读取这个，或者回退到 Primary

# 3. 名字到图标的映射
CATEGORY_TO_ICON = {m["name"]: m.get("icon", "📅") for m in FAMILY_MEMBERS}
CATEGORY_TO_ICON["Family"] = "🏠"

# 4. 生成 System Prompt 中的角色描述字符串
def generate_role_description():
    desc = "Classify based on WHO:\n"
    for m in FAMILY_MEMBERS:
        desc += f"    - **{m['name']}**: {m['role']}\n"
    desc += "    - **Family**: Shared events for everyone."
    return desc

ROLE_DESCRIPTION_STR = generate_role_description()
...
def validate_and_fix_payload(data: dict) -> Tuple[bool, str]:
    ...
    if cat not in VALID_CATEGORIES:
        # 默认回退到列表中的第一个人（通常是 Default/Father/Mother）
        default_person = FAMILY_MEMBERS[0]["name"]
        logger.warning(f"⚠️ Unknown category '{cat}', fallback to '{default_person}'")
        data['category'] = default_person
    
    is_all_day = data.get('is_all_day', False)
    try:
        if is_all_day:
            datetime.strptime(data['start_time'], '%Y-%m-%d')
            if data.get('end_time'): datetime.strptime(data['end_time'], '%Y-%m-%d')
        else:
            datetime.strptime(data['start_time'], '%Y-%m-%d %H:%M:%S')
            if data.get('end_time'): datetime.strptime(data['end_time'], '%Y-%m-%d %H:%M:%S')
    except ValueError:
        return False, "时间格式错误"
    return True, "OK"
...
async def create_calendar_event(event_data, user_current_tz):
    ...
    # [核心修改] 动态查找日历 ID
    # 1. 获取 Category
    cat = event_data.get('category', FAMILY_MEMBERS[0]['name'])
    
    # 2. 查找对应的环境变量 Key
    env_key = CATEGORY_TO_ENV.get(cat, "GOOGLE_CALENDAR_ID") # 找不到默认去 Primary
    
    # 3. 从环境变量读取实际 ID
    tid = os.getenv(env_key)
    
    # 4. 如果对应 ID 没配，回退到 Primary (GOOGLE_CALENDAR_ID)
    if not tid:
        tid = os.getenv("GOOGLE_CALENDAR_ID", "primary")
        logger.warning(f"⚠️ Category '{cat}' mapped to env '{env_key}' but it's empty. Fallback to Primary.")
...
def get_system_prompt(user_tz, is_explicit_event_mode=False):
    ...
    # [核心修改] 注入动态生成的角色描述
    members_list_str = "|".join([m['name'] for m in FAMILY_MEMBERS] + ["Family"])
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_auth(update): return
    # 动态显示 Bot 服务对象
    default_name = FAMILY_MEMBERS[0]['name']
    msg = (
        f"🤖 **Calendar Bot v2.8 (Configurable)**\n"
...
async def status_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 显示当前加载的家庭成员
    members_str = ", ".join([m['name'] for m in FAMILY_MEMBERS])

    msg = (
        f"📊 **System Status (v2.8)**\n\n"
        f"🌍 时区: `{tz_str}`\n"
        f"🕰 时间: `{now}`\n"
        f"👪 成员: `{members_str}`\n"
        f"🔑 Creds: {creds_ok}\n"
...
async def restart_singbox_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success: await status_msg.edit_text(f"{msg}\n⏳ 请等待 1-2 分钟让服务重新上线。")
    else: await status_msg.edit_text(f"⚠️ 操作失败: {msg}")
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if update.message.photo:
            ...
            resp = await client.chat.completions.create(
                model=MODEL_NAME,
                messages=[{"role": "system", "content": sys_prompt}, {"role": "user", "content": text_content}],
                temperature=0.3
            )
        ...
        if succ:
            if is_all_day:
                ...
                weekday = get_chinese_weekday(dts)
                # [核心修改] 动态获取 Icon
                icon = CATEGORY_TO_ICON.get(res.get('category'), '📅')
...
if __name__ == '__main__':
    ...
    if tk := os.getenv("TELEGRAM_TOKEN"):
        ...
        print("✅ Calendar Bot v2.8 (Configurable Family) Started...")
        app.run_polling()